...
class CustomPaginatedDataTableState extends State<CustomPaginatedDataTable> {
  ...
  List<CustomDataRow> _getRows(int firstRowIndex, int rowsPerPage) {
    ...
    for (int index = firstRowIndex; index < nextPageFirstRowIndex; index += 1) {
      ...
      if (index < _rowCount || _rowCountApproximate) {
        row = _rows.putIfAbsent(widget.allSortedIndexList[index], () => widget.source.getRow(widget.allSortedIndexList[index]));
  ...
  void _handleSort(column){
    print(sortedColumnIndex);
  ...
  Widget build(BuildContext context) {
    if(widget.customColumnsIndex == null){
      widget.customColumnsIndex = List.generate(widget.columns.length, (index) => index);
    }
    if (widget.allSortedIndexList.isEmpty){
      widget.allSortedIndexList = List.generate(widget.source.rowCount, (index) => index);
    }
              ...
              SingleChildScrollView(
                ...
                child: ConstrainedBox(
                  ...
                  child: CustomDataTable(
                    key: _tableKey,
                    customColumnsIndex: widget.customColumnsIndex ?? List.generate(widget.columns.length, (index) => index),
                    sortedIndexList: widget.sortedIndexList ?? List.generate(widget.rowsPerPage, (index) => index),
